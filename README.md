...
repository into your computer and follow the instructions at the [bottom of this page](#using-BetaSeries-as-a-template).
...
`BetaSeries/BetaSeries.py`. This code provided in this file is intentionally rather
...
We place the module code in a file called `BetaSeries.py` in directory called
`BetaSeries`. This structure is a bit confusing at first, but it is a simple way
to create a structure where when we type `import BetaSeries as sb` in an
interactive Python session, the classes and functions defined inside of the
`BetaSeries.py` file are available in the `sb` namespace. For this to work, we
...
In the case of the BetaSeries module, the main classes defined at the bottom of
...
Either way, you can create a `BetaSeries/data` folder in which you can
...
(typically, something like our `BetaSeries/tests/test_BetaSeries.py`). Within each
...
have one such test in `BetaSeries/tests/test_BetaSeries.py` called
...
the top-level directory of the repository (e.g. `/Users/arokem/code/BetaSeries`),
and type:
...
will see a message such as:
    ...
    BetaSeries/tests/test_BetaSeries.py .F...
    ...
    BetaSeries/tests/test_BetaSeries.py:49: AssertionError
...
To document `BetaSeries` we use the [sphinx documentation
...
![RTD conf](https://github.com/uwescience/BetaSeries/blob/master/doc/_static/RTD-advanced-conf.png)

 http://BetaSeries.readthedocs.org/en/latest/
...
A `BetaSeries/version.py` contains all of the information needed for the
installation and for setting up the [PyPI
page](https://pypi.python.org/pypi/BetaSeries) for the software. This
...
`BetaSeries` against Python 2.7, Python 3.3 and Python 3.4). It includes steps
that need to be taken before installation of the software. For example,
installation of the software dependencies. For `BetaSeries`, we use the
...
[here](https://github.com/uwescience/BetaSeries/blob/master/scripts/Figure1.ipynb).


### Using `BetaSeries` as a template
...
computer under the name you will want your project to have:
	...
	git clone https://github.com/uwescience/BetaSeries smallish
	cd smallish
...
Next, you will want to move `BetaSeries/BetaSeries.py` to be called
`smallish/smallish.py`

	git mv BetaSeries smallish
	git mv smallish/BetaSeries.py smallish/smallish.py
	git mv smallish/tests/test_BetaSeries.py smallish/tests/test_smallish.py

Make a commit recording these changes. Something like:

	git commit -a -m"Moved names from `BetaSeries` to `smallish`"

You will probably want to remove all the example data:

	git rm smallish/data/*
	git commit -a -m"Removed example `BetaSeries` data"

Possibly, you will want to add some of your own data in there.

You will want to edit a few more places that still have `BetaSeries` in them. Type
the following to see where all these files are:

	git grep BetaSeries

You can replace `BetaSeries` for `smallish` fastly with:

	git grep -l 'BetaSeries' | xargs sed -i 's/BetaSeries/smallish/g'
...
Edit all the mentions of `BetaSeries` in the `BetaSeries/__init__.py` file, and in
the `BetaSeries/version.py` file as well.
